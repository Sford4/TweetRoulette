...
export default class Sudoku {
	...
	initialize() {
        ...
        */
		...
		SQUARE_PEERS_MAP = this._get_square_peers_map(SQUARES, SQUARE_UNITS_MAP);
	}
	...
	generate(difficulty, unique) {
        ...
        */
		...
		// console.log('candidates at 109', candidates);
		// For each item in a shuffled list of squares
		var shuffled_squares = this._shuffle(SQUARES);
		// console.log('SQUARES at 112', SQUARES);
		for (var si in shuffled_squares) {
			...
			if (single_candidates.length >= difficulty && this._strip_dups(single_candidates).length >= 8) {
				// if (this._strip_dups(single_candidates).length >= 8) {
				var board = '';
				var givens_idxs = [];
        ...
        */
		...
		if (!reverse) {
			for (var vi in min_candidates) {
				...
				var candidates_next = this._search(this._assign(candidates_copy, min_candidates_square, val));
			...
			for (var vi = min_candidates.length - 1; vi >= 0; --vi) {
				...
				// TODO: Implement a non-rediculous deep copy function
				var candidates_copy = JSON.parse(JSON.stringify(candidates));
				var candidates_next = this._search(this._assign(candidates_copy, min_candidates_square, val), reverse);
...
//     console.log("potential board: " + output);
// }

// "build": "tsc main.ts && browserify main.js > build.full.js && uglifyjs --compress --mangle -- build.full.js > build.js",
